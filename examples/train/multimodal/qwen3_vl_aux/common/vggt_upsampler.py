...
from .vggt_head_act import activate_head
...
class VggtUpsampler(nn.Module):
    ...
    def __init__(
        ...
        super(VggtUpsampler, self).__init__()
    ...
    def _forward_impl(
        ...
        for idx, layer_idx in enumerate(self.intermediate_layer_idx):
            ...
            if actual_tokens != expected_tokens:
                raise ValueError(
                    'VggtUpsampler token/grid mismatch: '
                    f'layer_idx={layer_idx}, actual_tokens={actual_tokens}, expected_tokens={expected_tokens}, '
                    f'patch_h={patch_h}, patch_w={patch_w}, image_hw=({H}, {W}), '
                    f'patch_size={self.patch_size}, batch={B}, frames={S}, patch_start_idx={patch_start_idx}'
                )
